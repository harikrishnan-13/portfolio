    ...
    <div className="min-h-screen px-4 sm:px-6 py-10 sm:py-12 text-white relative overflow-hidden">
      ...
      <div className="relative max-w-5xl mx-auto border-l-2 border-blue-500/40 pl-4 sm:pl-6 space-y-10 sm:space-y-14">
        ...
        {education.map((edu, index) => (
          <motion.div
            key={index}
            className="relative bg-white/5 backdrop-blur-xl border border-gray-800 p-4 sm:p-6 rounded-xl shadow-lg hover:shadow-blue-500/20 transition"
          ...
          >
            ...
            {/* Timeline Dot */}
            <motion.div
              className="absolute -left-6 sm:-left-9 top-6 w-4 h-4 sm:w-5 sm:h-5 bg-blue-500 rounded-full border-4 border-gray-900"
            ...
            <h2 className="text-lg sm:text-xl font-semibold flex items-start sm:items-center gap-2">
              <FaGraduationCap className="text-blue-400 mt-1 sm:mt-0" />
              {edu.degree}
            </h2>

            {/* Institution */}
            <p className="text-blue-400 mt-1 flex items-start sm:items-center gap-2 text-sm sm:text-base">
              <FaSchool className="mt-1 sm:mt-0" />
              {edu.institution}
            </p>

            {/* Duration + Grade */}
            <div className="flex flex-col sm:flex-row sm:flex-wrap gap-2 sm:gap-4 text-xs sm:text-sm text-gray-400 mt-2">
            ...
            {edu.degree.includes("HSLC") && (
              <motion.div
                className="mt-4 inline-flex items-center gap-2 text-xs sm:text-sm bg-purple-600/20 text-purple-400 px-3 py-1 rounded-full border border-purple-500/30"
